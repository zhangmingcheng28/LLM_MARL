...
from Nnetworks_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import *
...
from memory_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import ReplayMemory, Experience
...
class MADDPG:
    def __init__(self, actor_dim, critic_dim, dim_act, actor_hidden_state_size, gru_history_length, n_agents,
                 ...
                 use_selfATT_with_radar, use_allNeigh_wRadar, own_obs_only, normalizer, use_nearestN_neigh_wRadar, device):
        self.args = args
        self.mode = args.mode
        self.actors = []
        self.critics = []
        ...
        for agent in range(var_iteration):
            ...
            if full_observable_critic_flag:
                ...
                if use_GRU_flag:
                    ...
                    non_final_next_actions, head_record  = self.actors_target([non_final_next_states_actorin[0],
            ...
            if full_observable_critic_flag:
                ...
                if use_GRU_flag:
                    ...
                    action_i, _ = self.actors([stacked_elem_0, stacked_elem_2, cur_mask_batch])
                    ac = action_i
    ...
    def choose_action(self, state, cur_total_step, cur_episode, step, mini_noise_eps, noise_start_level, actor_hiddens, use_allNeigh_wRadar, use_selfATT_with_radar, own_obs_only, use_nearestN_neigh_wRadar, all_agent_dict, step_head_selection_mask, noisy=True, use_GRU_flag=False):
        ...
        for i in range(self.n_agents):
            ...
            if use_GRU_flag:
                ...
                mask_encoding_numpy = mask_encoding.detach().cpu().numpy()
                step_head_selection_mask['x']['mask'] = mask_encoding_numpy[0,:]
                step_head_selection_mask['y']['mask'] = mask_encoding_numpy[1,:]
                act, head_selection_indicator = self.actors([sb.unsqueeze(0), sb_grid.unsqueeze(0), mask_encoding])
                x_head_indicator, y_head_indicator = head_selection_indicator
                if x_head_indicator == 'Not_any_head':
                    assert "None of the head selected"
                else:
                    step_head_selection_mask['x'][x_head_indicator] = 1
                if y_head_indicator == 'Not_any_head':
                    assert "None of the head selected"
                else:
                    step_head_selection_mask['y'][x_head_indicator] = 1
        ...
        return actions.data.cpu().numpy(), noise_value, gru_history_input.squeeze(0).data.cpu(), act_hn.data.cpu(), agent_masks_record, step_head_selection_mask  # NOTE: tensor.data.cpu() is to make the tensor's "is_leaf" = True, this also prevent the error message on line "retain_graph=True"
