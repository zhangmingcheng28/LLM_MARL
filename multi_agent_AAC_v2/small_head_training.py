...
class AvoidMaskNN_with_att(nn.Module):
    def __init__(self, num_segments=10):
        ...
        self.q_projection = nn.Linear(num_segments+1, 128)
        self.k_projection = nn.Linear(num_segments+1, 128)
        self.v_projection = nn.Linear(num_segments+1, 128)
...
# save_path = "./mask_model_fully_masked_wTanh_v2.pth"
save_path = "./mask_model_fully_masked_woTanh_v2.pth"
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % eval_interval == 0:
        ...
        for _ in range(num_eval_runs):
            ...
            # Find the nearest unmasked segment boundaries
            min_distances_to_mins, min_indices_to_mins = distances_to_unmasked_mins.min(dim=1)  # Shape: (num_selected,)
            min_distances_to_maxs, min_indices_to_maxs = distances_to_unmasked_maxs.min(dim=1)  # Shape: (num_selected,)

            # Find the overall nearest unmasked boundary
            is_closer_to_min = min_distances_to_mins <= min_distances_to_maxs  # Shape: (num_selected,)
            nearest_boundary_distances = torch.where(is_closer_to_min, min_distances_to_mins, min_distances_to_maxs)
            nearest_boundary_indices = torch.where(is_closer_to_min, min_indices_to_mins, min_indices_to_maxs)

            # Verify if output lies within the nearest unmasked segment
            # Get the nearest unmasked segment boundaries for each output
            nearest_unmasked_seg_mins = filtered_unmasked_seg_mins[
                torch.arange(len(filtered_unmasked_seg_mins)), nearest_boundary_indices]
            nearest_unmasked_seg_maxs = filtered_unmasked_seg_maxs[
                torch.arange(len(filtered_unmasked_seg_maxs)), nearest_boundary_indices]

            # Check if outputs lie within the nearest unmasked segments
            output_within_nearest_unmasked = (output_vals >= nearest_unmasked_seg_mins) & (
                        output_vals < nearest_unmasked_seg_maxs)
            count_true_output_within_nearest = output_within_nearest_unmasked.sum().item()
            count_false_output_within_nearest = output_within_nearest_unmasked.numel() - count_true_output_within_nearest

            # success_count = success_count + count_true_output_within_nearest  # condition 3 success count

        # Calculate and print success rate
        success_rate_for_one_eps = success_count / (batch_size*num_eval_runs)
