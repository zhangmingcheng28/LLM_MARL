...
from parameters_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import initialize_parameters
from maddpg_agent_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import MADDPG
from utils_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import *
from grid_env_generation_newframe_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import env_generation
from env_simulator_randomOD_radar_sur_drones_N_Model_use_tdCPA_forV2_changeskin_mask_ablation_v2 import env_simulator
...
def main(args):
    ...
    # # Remove the last element from each sub-list
    # modified_data = [sub_list[0] for sub_list in OD_eta_record]
    # import msgpack
    # with open("5AC_cur_eva_fixedAR_OD.msgpack", "wb") as file:
    #     file.write(msgpack.packb(modified_data))
    ...
    if args.mode == "train":
        ...
        file_name = '/home/bizhao/simulation_data/' + str(current_date) + '_' + str(formatted_time)
    ...
    all_episode_head_selection_mask = {}
    while episode < args.max_episodes:  # start of an episode
        ...
        current_eps_status = []
        current_eps_head_selection_mask = []
        while True:  # start of a step
            if args.mode == "train":
                ...
                current_eps_head_selection_mask.append(step_head_selection_mask)
                traj_step_list = []
                ...
                if True in episode_decision:  #  --- end of an episode starts here ---
                    ...
                    all_episode_head_selection_mask[episode] = current_eps_head_selection_mask
                    ...
                    # if episode % 1 == 0 and args.mode == "train":
                        ...
                        with open(plot_file_name + '/all_episode_head_selection_mask.pickle', 'wb') as handle:
                            pickle.dump(all_episode_head_selection_mask, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    if args.mode == "train":  # only save pickle at end of training to save computational time.
        ...
        with open(plot_file_name + '/all_episode_each_step_status.pickle', 'wb') as handle:
            pickle.dump(eps_status_record, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(file_name + '/GFG.csv', 'w', newline='') as f: # reward at each episode
            # using csv.writer method from CSV package
            write = csv.writer(f)
            for item in score_history:
                write.writerow([item])
        with open(file_name + '/goal_reaching.csv', 'w') as f: # goal reach at each 1000 episode
            write = csv.writer(f)
