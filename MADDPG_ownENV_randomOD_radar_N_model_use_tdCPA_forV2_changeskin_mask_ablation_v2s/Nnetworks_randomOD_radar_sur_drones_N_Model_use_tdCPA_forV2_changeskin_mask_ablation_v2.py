...
class LLM_actor_pretrained_head(nn.Module):
    ...
    def forward(self, cur_state):
        ...
        masked_out_a, head_selection_indicator_x = head_selection(NN_selected_out_action[:, 0].unsqueeze(1), self.pretrained_inside, self.pretrained_all_mask, self.pretrained_not_inside, outside_masked_segments_a, all_segments_masked_a, inside_masked_segments_a, current_mask_a)
        masked_out_b, head_selection_indicator_y = head_selection(NN_selected_out_action[:, 1].unsqueeze(1), self.pretrained_inside, self.pretrained_all_mask, self.pretrained_not_inside, outside_masked_segments_b, all_segments_masked_b, inside_masked_segments_b, current_mask_b)
        # ------ end of linkage ---------
        final_output = torch.cat([masked_out_a, masked_out_b], dim=1)
        return final_output, (head_selection_indicator_x,head_selection_indicator_y)
