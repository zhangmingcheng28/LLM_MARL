...
def tint_image(image, rgb_color):
    ...
    r, g, b, a = image.split()  # Split into RGBA channels

    # Create solid color image based on the tint color
    color_image = Image.new("RGBA", image.size, (
        int(rgb_color[0] * 255),
        int(rgb_color[1] * 255),
        int(rgb_color[2] * 255),
        255
    ...
    # Blend the color image with the original image using the alpha channel
    tinted_image = Image.composite(color_image, image, a)
    return tinted_image
...
def animate(frame_num, ax, env, trajectory_eachPlay):
    ...
    plt.xlabel("X axis")
    plt.ylabel("Y axis")
    aircraft_svg_path = r'/home/bizhao/MADDPG_ownENV_randomOD_radar_N_model_use_tdCPA_forV2_changeskin_mask_ablation_v2s/pictures/Aircraft.svg'  # Replace with your SVG path
...
def view_traj_by_scatter(env, trajectory_eachPlay, save_path=None, max_time_step=None):
    max_time_step = None
    save_path = None
    ...
    fig, ax = plt.subplots(1, 1)
    colors = [
        (0.5, 0, 0.5),  # Purple, agent_1, 0th agent in the python counter
        (0.2, 0.8, 0.6),  # Lime
        (1, 0, 0),  # Red, agent_3
        (0, 1, 0),  # Green
        (0, 0, 1),  # Blue, agent4
    ...
    for cloud_idx, cloud_agent in enumerate(env.cloud_config):
        for trajectory_idx in range(fade_end + interval):
            if trajectory_idx >= max_time_step:
                break
            if trajectory_idx % interval == 0:
                # Define the fixed center
                if max_time_step == len(trajectory_eachPlay):
                    center_x, center_y = cloud_agent.trajectory[trajectory_idx].x, cloud_agent.trajectory[
                        trajectory_idx].y
                else:
                    center_x, center_y = cloud_agent.trajectory[max_time_step].x, cloud_agent.trajectory[
                        max_time_step].y
                if trajectory_idx == fade_end:
                    plt.text(center_x, center_y, 't=' + str(trajectory_idx))
                # ___add boundary circle for clouds---
                # cloud_centre = Point(center_x, center_y)
                # cloud_poly = cloud_centre.buffer(cloud_agent.radius)
                # matp_poly = shapelypoly_to_matpoly(cloud_poly, False, 'blue')  # the 3rd parameter is the edge color
    ...
    fade_start, fade_end = 35, 70
    alpha_values = np.linspace(0.2, 1.0, fade_end - fade_start + 1)  # Gradual fade from 0.2 to 1.0
    for agentIDX, agent in env.all_agents.items():
        ...
        for trajectory_idx in range(max_time_step):
            ...
            if trajectory_idx >= fade_start and trajectory_idx <= fade_end and trajectory_idx % 5 == 0:
                # Calculate alpha based on trajectory_idx
                alpha_idx = trajectory_idx - fade_start
                ...
                transform = Affine2D().rotate_deg_around(x, y, heading - 90) + ax.transData
                # Assign agent-specific color
                chosen_color = colors[agentIDX]
                tinted_image = tint_image(png_image, chosen_color)
                ...
                grid_mat_SCir.set_zorder(2)
                grid_mat_SCir.set_alpha(current_alpha)  # Apply fading alpha
                if agent.ini_eta == None or agent.ini_eta < trajectory_idx:
                    if agentIDX == 4 or agentIDX == 0 or agentIDX == 2:
                        ax.add_patch(grid_mat_SCir)
                        ax.imshow(tinted_image, extent=img_extent, zorder=10, transform=transform, alpha=current_alpha)
                        # below for big figure x,y, max bound
                        if agentIDX == 4:
                            if trajectory_idx == 45:
                                plt.text(x - 3, y + 6, 't=' + str(trajectory_idx), color=chosen_color)
                            if trajectory_idx == 50:
                                plt.text(x - 1, y + 5, 't=' + str(trajectory_idx), color=chosen_color)
                            if trajectory_idx == 55:
                                plt.text(x, y + 6, 't=' + str(trajectory_idx), color=chosen_color)
                        if agentIDX == 0:
                            if trajectory_idx == 50:
                                plt.text(x + 3.5, y - 2, 't=' + str(trajectory_idx), color=chosen_color)
                            if trajectory_idx == 55:
                                plt.text(x - 20, y, 't=' + str(trajectory_idx), color=chosen_color)
                        if agentIDX == 2:
                            if trajectory_idx == 45:
                                plt.text(x, y - 8, 't=' + str(trajectory_idx), color=chosen_color)
                            if trajectory_idx == 50:
                                plt.text(x - 22, y - 4, 't=' + str(trajectory_idx), color=chosen_color)
                            if trajectory_idx == 55:
                                plt.text(x - 8.5, y + 5, 't=' + str(trajectory_idx), color=chosen_color)
                        # end for big figure x,y, max bound
                        # for cropped smalled bound
                        # if agentIDX == 4:
                        #     if trajectory_idx == 45:
                        #         plt.text(x-3, y+6, 't=' + str(trajectory_idx), color=chosen_color)
                        #     if trajectory_idx == 50:
                        ...
                        #         plt.text(x+3.5, y-2, 't=' + str(trajectory_idx), color=chosen_color)
                        #     if trajectory_idx == 55:
                        #         plt.text(x-13, y, 't=' + str(trajectory_idx), color=chosen_color)
                        # if agentIDX == 2:
                        #     if trajectory_idx == 45:
                        #         plt.text(x, y-8, 't=' + str(trajectory_idx), color=chosen_color)
                        #     if trajectory_idx == 50:
                        #         plt.text(x-14, y-2, 't=' + str(trajectory_idx), color=chosen_color)
                        #     if trajectory_idx == 55:
                        #         plt.text(x-5, y+6, 't=' + str(trajectory_idx), color=chosen_color)
                        # end for cropped smalled bound
    ...
    # plt.xlim(80, 180)
    # plt.ylim(30, 130)
    # Set the axis limits to restrict the visible area
    # ax.set_xlim(80, 180)
    # ax.set_ylim(30, 130)
    # Define axis limits
    x_min, x_max = env.bound[0], env.bound[1]
    # x_min, x_max = 80, 180
    y_min, y_max = env.bound[2], env.bound[3]
    # y_min, y_max = 30, 130

    # Set axis limits to restrict visible content
    ax.set_xlim(x_min, x_max)
    ax.set_ylim(y_min, y_max)
    # Manually remove ALL text elements (ax.texts and plt.text) outside the x-range
    # all_texts = ax.texts + [child for child in fig.get_children() if isinstance(child, plt.Text)]
    # for text in all_texts:
    #     text_x, text_y = text.get_position()  # Get the (x, y) position of the text
    #     if text_x < x_min or text_x > x_max:  # Check if text is outside the x-range
    #         text.set_visible(False)  # Hide text outside the x-range
    # # Add a clipping rectangle for all elements
    # clip_rect = Rectangle((x_min, y_min), x_max - x_min, y_max - y_min, transform=ax.transData, linewidth=0)
    # ax.add_patch(clip_rect)
    #
    # # Clip all images and patches
    ...
    # ax.set_xticks([])
    # ax.set_yticks([])
    # plt.savefig(svg_path, format="svg", bbox_inches=ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted()))
